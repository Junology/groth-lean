...
definition hom_eval (α β : Type _) [model binary_module α] [model binary_module β] (a : α) : morphism binary_module (hom α β) β :=
{
  val := λ f, f.val a,
  property :=
    begin
...
#print axioms hom_eval

--- For every pair of `binary_module`s `α` and `β`, the type of homomorphisms `hom α β` is a model of `binary_module` up to `funext`.
definition hom_model_funext (α β : Type _) [model binary_module α] [model binary_module β] : ∀ {n : ℕ} (r : binary_module.rel n) (var : finord n → hom α β) (a : α), ((binary_module.rel_lhs r).elim (@premodel.act binary_module (hom α β) _) var).val a = ((binary_module.rel_rhs r).elim (@premodel.act binary_module (hom α β) _) var).val a :=
  begin
    intros n r var a,
    let hp := @optree.elim_funap binary_module.ops (finord n) (hom α β) β (@premodel.act binary_module (hom α β) _) (@premodel.act binary_module β _) var (hom_eval α β a).val (hom_eval α β a).property,
    delta hom_eval at hp,
    dsimp [subtype.val,function.comp] at hp,
    rw [hp,hp],
    rw [model.axiom_eq],
  end

#print axioms hom_model_funext

namespace unsafe

--- For every pair of `binary_module`s `α` and `β`, the type of homomorphisms `hom α β` is a model of `binary_module`.
definition hom_model (α β : Type _) [model binary_module α] [model binary_module β] : model binary_module (hom α β) :=
{
  haxiom := λ _ r var, subtype.eq (funext (@hom_model_funext α β _ _ _ r var))
}

#print axioms hom_model
