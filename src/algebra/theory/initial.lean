...
definition fixed (th : theory) [ht : has_trivial_init th] (α : Type _) [model th α] : α := ((@is_initial.elim th unit _ ht.init_unit) α _).to_fun ()

--- Proof that `fixed th α` is closed under operations in the theory `th`.
theorem fixed_op (th : theory) [ht : has_trivial_init th] (α : Type _) [model th α] : ∀ n μ, @premodel.act th α _ n μ (vect.repeat (fixed th α) n) = fixed th α :=
  begin
    intros n μ,
    dsimp [fixed],
...
definition prod_inl (th : theory) [has_trivial_init th] {α β : Type _} [model th α] [model th β] : morphism th α (α×β) :=
{
  ...
  hact :=
    begin
      ...
      have : vect.map (λ _, fixed th β) as = vect.repeat (fixed th β) _, {
        clear μ,
...
definition prod_inr (th : theory) [has_trivial_init th] {α β : Type _} [model th α] [model th β] : morphism th β (α×β) :=
{
  ...
  hact :=
    begin
      ...
      rw [←this,vect.map_id]; clear this,
      have : vect.map (λ _, fixed th α) as = vect.repeat (fixed th α) _, {
        clear μ,
        induction as with _ a as hind,
        case vect.nil { refl },
        case vect.cons {
